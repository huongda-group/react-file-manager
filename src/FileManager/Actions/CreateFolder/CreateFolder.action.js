...
const CreateFolderAction = ({ filesViewRef, file, onCreateFolder, triggerAction }) => {
    ...
    useEffect(() => {
        ...
        if (outsideClick.ref?.current) {
            ...
            const bottomAvailableSpace = filesContainerRect.bottom - (nameInputContainerRect.top + nameInputContainer.clientHeight);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(NameInput, { nameInputRef: outsideClick.ref, maxLength: maxNameLength, value: folderName, onChange: handleFolderNameChange, onKeyDown: handleValidateFolderName, onClick: (e) => e.stopPropagation(), ...(activeLayout === "list" && { rows: 1 }) }), folderNameError && (_jsx(ErrorTooltip, { message: folderErrorMessage, xPlacement: errorXPlacement, yPlacement: errorYPlacement }))] }));
};
