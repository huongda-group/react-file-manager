...
// Global cache to prevent creating thousands of React elements on every render
// FileItem component calls this hook twice per instance. In a list of 1000 files,
// this prevents creating 68,000 React elements on every single render.
const iconCache: Record<number, Record<string, ReactElement>> = {};

export const useFileIcons = (size?: number): Record<string, ReactElement> => {
  return useMemo(() => {
    const sizeKey = size || 0;

    if (iconCache[sizeKey]) {
      return iconCache[sizeKey];
    }
