...
| `onDelete`               | (files: Array<[File](#-file-structure)>, trash: boolean) => void                                                                                | A callback function is triggered when one or more files or folders are deleted. The `trash` boolean indicates if it should be moved to trash or deleted permanently. |
...
| `onRename`               | (file: [File](#-file-structure), newName: string, parentPath?: string) => void \| Promise<void>                                        | A callback function triggered when a file or folder is renamed. Receives the file object, the new name, and optionally the parent directory path. If it returns a Promise, the UI will wait for it to resolve before closing the input field.                                                                                                                                                                                                                                                                                                                                                           |
