...
| `onUpload` | (file: File) => Promise<any> | (Optional) A callback function triggered when a file is selected for upload. It receives the selected `file` object and expects a Promise that resolves with the uploaded file details (to be added to the file list) or rejects with an error. This replaces `fileUploadConfig` to give full control over the upload process (e.g., using `axios`, `fetch`, or checking constraints before uploading). |
...
| `onCompress`             | (files: IFile[], callback: (result: { status: boolean; message: string }) => void) => void                                                                                | (Optional) A callback function triggered when "Compress" is selected. Receives selected files and a callback to report status/message.                                                                                                                                                                                                                                                                                                                                                                                          |
| `onDecompress`           | (files: IFile[], callback: (result: { status: boolean; message: string }) => void) => void                                                                                | (Optional) A callback function triggered when "Decompress" is selected. Receives selected file and a callback to report status/message. Only shows for single file selection.                                                                                                                                                                                                                                                                                                                                                       |
| `onCreateFolder`         | (name: string, parentFolder: [File](#-file-structure)) => void \| Promise<void>                                   | A callback function triggered when a new folder is created. Use this function to update the files state to include the new folder under the specified parent folder using create folder API call to your server. If it returns a Promise, the UI will wait for it to resolve before closing the input field. |
...
| `onRename`               | (file: [File](#-file-structure), newName: string) => void \| Promise<void>                                        | A callback function triggered when a file or folder is renamed. If it returns a Promise, the UI will wait for it to resolve before closing the input field.                                                                                                                                                                                                                                                                                                                                                           |
| `onChmod`                | (files: Array<[File](#-file-structure)>, permissions: string) => void                                               | (Optional) A callback function triggered when the user changes file/folder permissions via the context menu. Receives the selected files and the new permissions as an octal string (e.g., "755", "644").                                                                                                                                                                                                                                                                                                |
...
```jsx
<FileManager files={files} theme="dark" />
```

Alternatively, you can add `data-theme="dark"` attribute to a container element:
